...
def test_quit():
    """input of quit halts program"""
    check50.run("python3 grocery.py").stdin("quit", prompt=False).exit(0)
...
def test_single_items():
    ...
    items = ["apple", "banana"]
    output = "1 APPLE\n1 BANANA"
    check50.run("python3 grocery.py").stdin(items[0], prompt=False).stdin(items[1], prompt=False).stdin("quit", prompt=False).stdout(regex(output), output, regex=True).exit()
