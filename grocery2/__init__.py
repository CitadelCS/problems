...
def exists():
    """grocery2.py exists"""
    check50.exists("grocery2.py")


@check50.check(exists)
def test_EOF():
    """input of EOF halts program"""
    check50.run("python3 grocery2.py").stdin(EOF, prompt=False).exit(0)
...
def test_single_items():
    ...
    items = ["apple", "banana"]
    output = "1 APPLE\n1 BANANA"
    check50.run("python3 grocery2.py").stdin(items[0], prompt=False).stdin(items[1], prompt=False).stdin(EOF, prompt=False).stdout(regex(output), output, regex=True).exit()
...
def test_single_and_multiple_items():
    ...
    output = "2 MANGO\n1 SUGAR"
    check50.run("python3 grocery2.py").stdin(items[0], prompt=False).stdin(items[1], prompt=False).stdin(items[2], prompt=False).stdin(EOF, prompt=False).stdout(regex(output), output, regex=True).exit()
